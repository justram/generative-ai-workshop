...
const TOOL_LIST = [
	{ name: `shell.date`, title: `讀取目前日期時間`, description: `回傳本機伺服器目前時間。只讀。`, risk: `read` },
	{ name: `shell.pwd_list`, title: `讀取目前資料夾`, description: `回傳教學 app 的工作目錄與前幾個檔案。只讀。`, risk: `read` },
	{ name: `runtime.node_version`, title: `讀取 Node.js 版本`, description: `回傳本機 Node.js 版本。只讀。`, risk: `read` },
	{ name: `project.package_summary`, title: `讀取 package.json 摘要`, description: `讀取 app 名稱、scripts 與 Electron 打包設定。只讀。`, risk: `read` },
	{ name: `notes.write_practice`, title: `寫入練習筆記`, description: `只允許寫入本機教學沙盒裡的 mcp-practice-note.txt。`, risk: `write` },
	{ name: `notes.read_practice`, title: `讀取練習筆記`, description: `讀取本機教學沙盒裡的 mcp-practice-note.txt。`, risk: `read` },
];
...
const EXAMPLES = [
	...
	},
	{
		...
		defaultPlan: {
			...
			arguments: { content: `- MCP server 可能讓模型讀取本機資料。\n- 能寫入或執行命令的工具要有人工確認。\n- 權限要限制在教學沙盒，不要直接接到重要資料。` },
		},
	},
	{
		title: `檢查剛剛的檔案`,
...
let MCPDemoInteractive = class extends DemoBase {
	constructor() {
		...
		this.session.setSystemPrompt(i18n(`你是一位教學助理。請根據工具結果回答，不要聲稱你直接碰到使用者電腦。`));
	...
	async loadToolList() {
		try {
			...
			this.mcpError = i18n(`本機 MCP 教學端點尚未連線。請重新啟動本機 app 或 dev server，讓 /api/mcp-demo 載入。`);
		}
	}
	...
	async runExample(example) {
		...
		if (!this.mcpConnected) {
			this.mcpError = i18n(`本機 MCP 教學端點未連線，這次不使用假資料替代。請重新啟動 app 後再試。`);
			return;
		}
		this.resetDemo();
		this.isRunning = true;
		...
		this.session.appendMessage({ role: `user`, content: [{ type: `text`, text: example.prompt }], timestamp: Date.now() });
...
${TOOL_LIST.map((tool) => `- ${tool.name}: ${tool.description}`).join(`\n`)}`);
	...
	appendVisibleToolMessages(plan, response) {
		...
		this.session.appendMessage({
			role: `assistant`,
			content: [{ type: `toolCall`, id: toolCallId, name: plan.tool, arguments: plan.arguments ?? {} }],
			api: `local-mcp-demo`,
			provider: `local`,
			model: `mcp`,
	...
	async generateFinalAnswer(example, plan, response) {
		...
		finalSession.setSystemPrompt(`你是繁體中文教學助理。請根據 MCP 工具結果回答學生原本問題，語氣自然精簡。最後用一句話點出 MCP 和前面工具呼叫的差別：前面的工具多半是 app 內建；MCP 是外部 server 先宣告工具清單，client 再用標準協定呼叫。不要提到 JSON。`);
		const prompt = `學生問題：
${example.prompt}
...
風險提示：${risk}`;
	...
	renderPipeline() {
		...
		const previewManifest =
			this.mcpToolManifest ?? {
				jsonrpc: `2.0`,
				id: 1,
				result: { tools: TOOL_LIST.map((tool) => ({ name: tool.name, description: tool.description })) },
			};
		const previewRequest = this.mcpRequest ?? { jsonrpc: `2.0`, id: 1, method: `tools/call`, params: { name: previewPlan.tool, arguments: previewPlan.arguments ?? {} } };
		...
		const elapsedMilliseconds = Math.max(0, (this.traceFinishedAt || this.traceNow || Date.now()) - (this.traceStartedAt || Date.now()));
		return x`
					...
					<div class="overflow-y-auto border-t border-border p-4 overscroll-contain" style="max-height:42vh">
						...
						<div class="grid gap-3">
							${this.renderPipelineStep(
								...
								0,
								i18n(`發現工具`),
								i18n(`MCP server 先回覆工具清單。這一步決定模型等一下「看得到」哪些能力，也決定權限邊界在哪裡。`),
							...
							${this.renderPipelineStep(
								...
								i18n(`MCP client 把工具名稱與參數包成 tools/call request。server 只應該執行自己允許的工具。`),
							...
							${this.renderPipelineStep(
								...
								3,
								i18n(`server 回傳結果`),
								i18n(`工具在外部 server 執行，結果再用標準格式回到 app。這個結果會進入模型上下文，成為回答的依據。`),
								compactResponse ? this.renderProtocolDetail(i18n(`tools/call response`), compactResponse) : x`<div class="text-xs text-muted-foreground">${i18n(`等待 MCP server 回傳結果。`)}</div>`,
							)}
							${this.renderPipelineStep(
								`answer`,
								4,
								i18n(`模型整理回答`),
								i18n(`模型不需要知道工具怎麼實作，只根據工具結果，用學生看得懂的方式回答原本問題。`),
								this.generatedAnswer
							...
							${this.renderPipelineStep(
								...
								5,
								i18n(`對照聊天區`),
								i18n(`聊天區只留下學生問題、工具卡、工具結果與模型回答；協定細節留在這個檢視裡，避免干擾一般對話。`),
								undefined,
								false,
							)}
						</div>
			...
			return x`
				<div class="rounded-md border ${done ? `border-border bg-muted/30` : `border-emerald-500/50 bg-emerald-500/10`} p-3 text-xs leading-6">
					...
					<div class="text-muted-foreground">
						${done
							? i18n(`請對照中央流程檢視與聊天區：協定細節留在流程裡，聊天區只保留學生真正需要看的互動。`)
							: i18n(`請特別看第 0 步：server 先宣告工具清單。這就是 MCP 和前面 app 內建工具最大的不同。`)}
					</div>
				</div>
			`;
		}
