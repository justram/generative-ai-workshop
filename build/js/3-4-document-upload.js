...
import { loadAttachment as a } from "./CodeBlock-SUyIenKs.js?v=msg-placeholder-1";
import { AgentInterface as o } from "./app-C9nW8ndw.js";
import { Badge as s, Card as c, CardContent as l, CardHeader as u, CardTitle as d } from "./Textarea-DCZnYrSo.js";
...
let b = class extends f {
	constructor() {
		...
		this.providerSystemPrompt = t(`你是一位文件問答服務中的助理。若服務供應商已把文件前處理成文字、表格或 JSON，請優先根據抽取內容回答；若使用者只上傳原始影像、沒有抽取內容，請明確說明你正在直接讀圖，並提醒欄位、金額、日期與單位仍需人工核對。若內容不足，請說明無法判斷。遇到金額、日期、編號、法規條文或承諾時，請保留原始格式與單位，不要自行補完。`);
		this.files = [
			{
				...
				prompt: t(`二零二二年使用權資產總額是多少？租賃負債流動與非流動各是多少？被質押的土地使用權賬面淨值是多少？請保留單位。`),
				learnerReference: t(`二零二二年使用權資產總額是人民幣 532,750 千元。租賃負債流動為人民幣 5,629 千元，非流動為人民幣 8,936 千元。被質押的土地使用權賬面淨值是人民幣 106,237,000 元。`),
				teaching: t(`教學重點：文件抽取後仍要小心單位。表格是「人民幣千元」，敘述段落是「人民幣元」，少看單位就會差一千倍。`),
			},
			{
				...
				prompt: t(`這個案件的違法單位是誰？兩項違法事實、處罰內容與決定日期分別是什麼？不要把處罰依據誤當成違法事實。`),
				learnerReference: t(`違法單位是陽維口腔診所。違法事實有兩項：未定期開展消毒與高壓滅菌效果檢測工作；過期藥品未按類別分置於專用包裝物及容器內。處罰內容是警告、罰款人民幣 3000.00 元，並責令立即改正。決定日期是 2019 年 12 月 6 日。`),
				teaching: t(`教學重點：這類公文欄位很長，模型常把「違法事實、法律依據、處罰內容」混成一段看似正確的摘要。要練的是欄位邊界。`),
			},
			{
				...
				datasetTask: t(`影像欄位抽取為 JSON`),
				prompt: t(`請列出這張收據的日期、收據號碼、賣方名稱、GST ID、總額與稅額。金額請照文件內容，不要自行補幣別。`),
				learnerReference: t(`日期：26/02/2018。收據號碼：BWMC02000047293。賣方名稱：Eco - Shop Marketing Sdn Bhd。GST ID：000313901056。總額：16.31。稅額：0.91。`),
				teaching: t(`教學重點：收據資料看起來短，但錯一個字元就可能對帳失敗。這題適合說明「摘要正確」和「欄位精準」是兩種不同能力。`),
			},
			{
				...
				prompt: t(`這份商品銷售明細的統計期間是什麼？前三筆「千禾」商品的國際條碼、品名、數量小計與金額小計分別是多少？請逐列回答。`),
				learnerReference: t(`統計期間是 2022-04-01 到 2022-04-30。前三筆千禾商品：1. 6928312913136，千禾特鮮生抽500ml，數量 20.00，金額 138.00。2. 6928312964664，千禾特香老抽500ml，數量 21.00，金額 144.54。3. 6928312988882，千禾黃豆醬油680m1，數量 21.00，金額 136.01。`),
				teaching: t(`教學重點：大型表格不是只要「看懂內容」，還要維持欄位對齊。模型若跳列、漏列或把相鄰欄位合併，答案會很像真的但其實不能用。`),
			},
		];
		this.session = new p();
		...
		this.unsubscribe = this.session.subscribe(e => {
			...
			if (n?.attachments && n.attachments.length > 0) {
				...
				if (!this.files.find(t => t.imageName === e.fileName)) {
					this.selectedFile = ``;
					if (!(this.selectedDocument?.isCustom && this.selectedDocument.displayName === e.fileName)) this.prepareCustomAttachmentPipeline(e);
					this.referenceAnswer = ``;
				}
			}
		});
	...
	getProviderSystemPrompt() {
		return this.isOcrMode()
			? t(`你是一位文件問答服務中的助理。服務供應商會先把文件前處理成文字、表格或 JSON。請優先根據抽取內容回答，並提醒欄位、金額、日期與單位仍需人工核對。若內容不足，請說明無法判斷。遇到金額、日期、編號、法規條文或承諾時，請保留原始格式與單位，不要自行補完。`)
			: t(`你是一位文件問答服務中的助理。這次服務不提供 OCR/Markdown 中間表示，模型會直接看原始影像或附件。回答時請明確說明你正在直接讀圖；若小字、欄位、金額、日期或單位看不清楚，請說不確定並建議人工核對，不要自行補完。`);
	}
	...
	handleEditorAttachmentsChange(e) {
		...
		const n = [...(e || [])].reverse().find(e => !this.files.find(t => t.imageName === e.fileName)) || e?.[0];
		if (!n) {
	...
	async prepareCustomAttachmentPipeline(e) {
		...
		this.selectedDocument = {
			isCustom: true,
			markdownName: this.isOcrMode() && e.extractedText ? t(`使用者上傳附件的抽取文字`) : t(`沒有中間文字`),
			...
			datasetTask: this.isOcrMode() && e.extractedText ? t(`附件處理器抽取文字`) : e.type === `image` ? t(`原生視覺輸入，沒有抽取文字`) : t(`附件讀取`),
			prompt: ``,
			learnerReference: ``,
			teaching: ``,
		};
			...
			${l(i`
				...
				<div class="mt-2 rounded-md border border-border bg-muted/30 p-2 text-[11px] leading-5 text-muted-foreground">
					${e
						...
						: t(`目前聊天框只會收到原始影像附件。流程會保留「沒有中間表示」這件事，避免把讀圖結果偽裝成 OCR。`)}
				</div>
			`)}
		`);
	}
	...
	renderProgressMarker(e, n, r) {
		if (n) return i`<div class="grid h-7 w-7 place-items-center rounded-full bg-emerald-500 text-xs font-bold text-black">✓</div>`;
		if (e) {
			return i`
			...
			return i`
				<div class="rounded-md border border-amber-500/40 bg-amber-500/10 p-3">
					...
					<div class="mt-1 text-muted-foreground">
						${this.isOcrMode()
							? t(`這份自訂附件沒有可用的本機 OCR/Markdown 中間表示。系統不會假裝抽取成功；若要完整觀察 OCR 流程，請選左側資料集範例。`)
							: t(`目前是原生視覺路徑：模型直接讀圖。這很方便，但沒有 Markdown/表格中間結果可以 debug。`)}
					</div>
				</div>
			`;
		}
			...
			<div>
				<div>
					${this.hasExtractedLayer()
						...
						: t(`已把原始影像與隱藏系統提示放入模型上下文；這次沒有可檢查的 OCR/Markdown 中間表示。`)}
				</div>
				${this.renderStepOutcome(
					t(`查看送進模型的服務規則`),
	...
	renderPipelineCard() {
		...
		const a = Math.max(0, (this.pipelineFinishedAt || this.pipelineNow || Date.now()) - (this.pipelineStartedAt || Date.now()));
				...
				<summary class="flex cursor-pointer select-none items-center justify-between gap-3 px-4 py-3">
					<div class="min-w-0 flex-1">
						...
						<div class="mt-1 text-xs leading-5 text-muted-foreground">
							${this.isOcrMode()
								? t(`這裡看的是 OCR 路徑：原始檔進來，服務先抽取文字/表格，再組上下文讓模型回答。`)
								: t(`這裡看的是原生視覺路徑：原始檔直接進模型上下文，沒有可檢查的 OCR/Markdown 中間表示。`)}
						</div>
					...
					<div class="grid gap-3">
						...
						${this.renderPipelineStep(
							...
							s?.isCustom && !this.extractedText
								...
								: this.isOcrMode()
									? t(`服務用 OCR、表格解析或版面分析產生中間表示。這一頁使用資料集已標註的抽取結果示範。`)
						...
						${this.renderPipelineStep(
							...
							this.hasExtractedLayer()
								...
								: t(`原始影像和隱藏系統提示一起進上下文；這時成本與錯誤來源要用 native vision 的方式理解。`),
						...
						${this.renderPipelineStep(
							`ready`,
								...
								o
									? s?.isCustom
										? this.hasExtractedLayer()
											? t(`下方聊天框保留了你上傳的附件與抽取文字；送出後，請檢查模型是否只根據可用內容回答。`)
											: t(`下方聊天框保留了你上傳的附件；送出後，請特別留意模型是否把看不清楚的欄位說得太確定。`)
										: this.hasExtractedLayer()
											? t(`下方聊天框已放入原始影像、抽取內容與問題；送出後，請對照左側參考答案檢查模型回答。`)
											: t(`下方聊天框已放入原始影像與問題，但沒有抽取內容；送出後，請對照左側參考答案檢查模型讀圖是否可靠。`)
									: t(`等待文件管線完成。`),
								false,
							)}
					</div>
				...
				<div class="space-y-3 text-xs leading-relaxed">
					...
					<div class="rounded-md border border-amber-500/40 bg-amber-500/10 p-3">
						...
						<div class="mt-1 text-muted-foreground">
							${e
								? n
									? t(`${e.displayName} 原圖 ${e.dimensions}。依 OpenAI API 影像 token 公式估算，GPT-5 high detail 約 70 + 每個 512px tile 140 tokens，這張約 ${e.gpt5HighTokens.toLocaleString(`en-US`)} tokens；GPT-4o high detail 約 85 + 每 tile 170 tokens，這張約 ${e.gpt4oHighTokens.toLocaleString(`en-US`)} tokens。low detail 是固定低解析預覽，便宜但不適合讀小字與表格。`)
									: t(`${e.displayName} 原圖尺寸仍在讀取中。若是影像，這裡會估算 high detail image tokens；若不是影像，成本主要取決於抽出的文字長度。`)
								: t(`選一份文件後，這裡會用該影像尺寸估算 high detail 的 image tokens。OpenAI API 文件列出 GPT-5 high detail 約 70 + 每個 512px tile 140 tokens；GPT-4o 約 85 + 每 tile 170 tokens。`)}
						</div>
