import { __decorate, i18n, r, t$1, x } from "./ThemeToggle-zh-tw7.js?v=proper-i18n-1";
import "./CodeBlock-SUyIenKs.js?v=msg-placeholder-1";
import { AgentInterface, getModel, registerToolRenderer } from "./app-C9nW8ndw.js";
import { Badge } from "./Textarea-DCZnYrSo.js";
...
import { DemoBase } from "./DemoBase-7724hyNv.js?v=section5-polish4";
import { AgentSession } from "./agent-session-CtmWvP9t.js";
import "./PreviewCodeToggle-BU28TKE5.js";
import { ArtifactsPanel } from "./artifacts-BhLRFxu0.js";
import { streamSimpleProxy } from "./proxy-client-DO8A5rUF.js";
...
function artifactChangeSummary(args) {
	...
	if (args.command === `rewrite`) return `以新的完整內容覆寫同一份檔案，適合大幅改寫或避免精準字串比對失敗。`;
	if (args.command === `update`) return `用 old_str / new_str 做局部替換；old_str 必須逐字相同才會成功。`;
...
function formatCommandPreview(command) {
	...
	if (typeof preview.content === `string`) preview.content = `已產生 ${preview.content.length} 字元，完整內容會交給 artifacts()。`;
	if (typeof preview.old_str === `string`) preview.old_str = `舊字串 ${preview.old_str.length} 字元`;
	if (typeof preview.new_str === `string`) preview.new_str = `新字串 ${preview.new_str.length} 字元`;
...
let ArtifactsToolDemo = class extends DemoBase {
	constructor() {
		...
		this.session.setSystemPrompt(i18n(`你正在教學示範 artifacts 工具。當使用者要求建立或修改文件、HTML、SVG、程式碼時，請務必呼叫 artifacts 工具，不要只把完整內容貼在聊天回覆裡。
...
- 不要假裝已經預覽成功；如果是 HTML，可以請使用者檢查預覽與 console。`));
		...
		this.artifactsPanel.attachmentsProvider = () =>
			this.session.state.messages.filter((message) => message.role === `user`).flatMap((message) => message.attachments || []);
	...
	getProcessingSteps() {
		...
		const base = [
				{ id: `planning`, label: i18n(`模型產生內容與工具指令`), description: i18n(`決定檔案名稱、操作方式，並產生要交給工具的內容。`) },
				{ id: `tool`, label: i18n(`本機工具寫入產物`), description: i18n(`artifacts() 只照模型給的 command、filename、content 寫入產物。`) },
				{ id: `inspect`, label: i18n(`檢查產物結果`), description: i18n(`學生要確認右側產物是否真的建立、更新，且內容符合任務。`) },
		];
		if (phase === `streaming`) {
			return [
					{ ...base[0], status: `active`, description: this.processingStage || i18n(`模型正在讀你的需求，可能會產生 artifacts 工具呼叫。`) },
		...
		if (phase === `tool` || phase === `recovering`) {
			return [
				{ ...base[0], status: `done` },
					{ ...base[1], status: `active`, description: this.processingStage || (phase === `recovering` ? i18n(`update 失敗時，改用 rewrite 修復同一份檔案。`) : base[1].description) },
				{ ...base[2], status: `waiting` },
			];
		}
		if (phase === `done`) {
			return [
				{ ...base[0], status: `done` },
				{ ...base[1], status: `done` },
				{ ...base[2], status: `done`, description: this.processingStage || base[2].description },
			];
		}
		return [
			{ ...base[0], status: `active`, description: this.processingStage || base[0].description },
			{ ...base[1], status: `waiting` },
			{ ...base[2], status: `waiting` },
		];
	}
	...
	renderProcessingCard() {
		...
		const elapsedMilliseconds = Math.max(0, (this.processFinishedAt || this.processNow || Date.now()) - (this.processStartedAt || Date.now()));
		...
		return [
			{
				...
				filename: `tool-risk-handout.md`,
					prompt: i18n(`請用 artifacts 工具建立一份 Markdown 講義，檔名用 tool-risk-handout.md。主題是「LLM 工具呼叫的三個風險」。請包含：一段開場、三個風險、每個風險一個生活例子、三題課後檢查題。`),
			},
			{
				id: `estimator`,
					title: i18n(`成本估算小工具`),
				...
				filename: `cost-estimator.html`,
					prompt: i18n(`請用 artifacts 工具建立一個單檔 HTML 小工具，檔名用 cost-estimator.html。功能：輸入 input tokens、output tokens、每百萬 input 價格、每百萬 output 價格，立即計算估計成本。請用深色背景、清楚標籤、不要使用外部套件。`),
			},
			{
				id: `checklist`,
					title: i18n(`提示注入檢查表`),
				...
				filename: `tool-safety-checklist.md`,
					prompt: i18n(`請用 artifacts 工具建立一份 Markdown 檢查表，檔名用 tool-safety-checklist.md。對象是產品團隊，用來審查「有工具呼叫的 AI 功能」。請分成：工具權限、資料來源、外送與寫入、失敗回復、上線前驗收。`),
			},
			{
				id: `diagram`,
				...
				filename: `agent-loop.svg`,
					prompt: i18n(`請用 artifacts 工具建立一張 SVG 圖解，檔名用 agent-loop.svg。內容要說明 AI 代理人的工具循環：使用者問題、模型判斷、工具呼叫、外部程式、結果回上下文。請讓文字清楚可讀。`),
			},
			{
				id: `revision`,
					title: i18n(`逐步修改練習`),
				...
				filename: `workshop-notes.md`,
					prompt: i18n(`請用 artifacts 工具建立一個 Markdown 檔案，檔名叫 workshop-notes.md。內容只要三點：工具讓模型補上外部能力、工具結果仍然是不可信上下文、高風險動作需要人工確認。建立後先停下來，接著我會要求你修改同一份檔案。`),
			},
		];
	}
...
${example.prompt}`;
		try {
			...
			this.processingPhase = `tool`;
				this.processingStage = i18n(`模型已交出工具參數；本機 artifacts() 準備建立 ${example.filename}。`);
			await wait(1400);
				this.processingStage = i18n(`本機 artifacts() 正在把內容寫進 ${example.filename}。`);
			const output = await this.appendAndExecuteToolCall({
				command,
					text: i18n(`我會建立 \`${example.filename}\`。下面是模型規劃出的 artifacts 工具呼叫，接著由本機工具執行。`),
...
`;
	...
	async planArtifactCommand(plannerPrompt, startedAt) {
		...
		let plannerText = ``;
		try {
			for await (const event of streamSimpleProxy(this.session.state.model, context, { authToken, maxTokens: 2200, signal: controller.signal })) {
			...
			if (!this.streamTimeoutId) {
				this.streamTimeoutId = setTimeout(() => {
					...
					setTimeout(() => {
						...
						this.session.appendMessage({
							role: `assistant`,
							content: [{ type: `text`, text: `這次 artifact 修改花太久，已自動停止。建議一次只改一件事，例如「只把開場改得更口語」或「只新增課堂驗收」。` }],
							timestamp: Date.now(),
						});
					}, 250);
				}, 60000);
	...
	async recoverFailedUpdateIfPossible(messages) {
		...
		const toolCall = messages
			...
			.find((part) => part.type === `toolCall` && part.id === failed.toolCallId && part.name === `artifacts`);
		if (!toolCall || toolCall.arguments?.command !== `update` || !toolCall.arguments?.filename || toolCall.arguments?.new_str === void 0) return;
		...
		const output = await this.appendAndExecuteToolCall({
			command: { command: `rewrite`, filename, content: toolCall.arguments.new_str },
	...
	async appendAndExecuteToolCall({ command, text, toolCallPrefix }) {
		...
		this.session.appendMessage({
			...
			content: [
				{ type: `text`, text },
				{ type: `toolCall`, id: toolCallId, name: `artifacts`, arguments: command },
			],
