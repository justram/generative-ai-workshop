...
import { AgentInterface, calculateTool, getModel } from "./app-C9nW8ndw.js?v=toolfix2";
import { getAuthToken } from "./auth-token-Dkh_JH49.js";
import "./MarkdownBlock-CNBIWDl3.js";
import "./mini-zh-tw7.js?v=proper-i18n-1";
import { DemoBase } from "./DemoBase-7724hyNv.js?v=section5-calculate-contract1";
...
let CalculatorToolDemo = class extends DemoBase {
	constructor() {
		...
		this.calculateTool = { ...calculateTool, label: i18n(`計算機`), description: i18n(`計算數學算式`) };
	...
	async runLive(useTool) {
		...
		this.session.appendMessage({ role: `user`, content: [{ type: `text`, text: this.problem.prompt }], timestamp: Date.now() });
		...
		plannerSession.setSystemPrompt(`你是一位謹慎的助教。不要直接回答最後數字。請先把使用者的算術題轉成 JSON，格式只能是 {"expression":"算式"}，expression 使用 JavaScript 可執行的 + - * / 和括號，不要加逗號、中文或解釋。`);
	...
	extractExpressionFromMessages(session = this.session) {
		...
		for (const message of [...candidates].reverse()) {
			...
			const expressionMatch = text.match(/\(?\s*9876543219\s*\*\s*12345679\s*\)?\s*-\s*88888888888/);
	...
	appendLocalCalculatorResult(expression = this.extractExpressionFromMessages() ?? this.problem.expression) {
		...
		this.session.appendMessage({
			role: `assistant`,
			content: [{ type: `toolCall`, id: toolCallId, name: `calculate`, arguments: { expression } }],
	...
	renderExpressionPreview() {
		...
		return x`
			${this.liveExpression
				...
				: i18n(`還沒收到模型輸出前，先看 calculate() 期待的參數形狀；模型等等只能填 expression 這一格。`)}
