...
List specific issues found and suggestions for improvement.`},{id:`improve`,name:`Step 3: Improve Based on Critique`,prompt:`Based on your critique, rewrite the executive summary to address all the issues you identified. Make it more impactful for senior executives.`}]}constructor(){super(),this.headerTitle=`Self-Correction - Iterative Improvement`,this.sectionId=`4.8`,this.currentStep=0,this.isProcessing=!1,this.completed=new Set,this.reportData=`Q3 2024 results: Revenue up 15%, new customers up 22%, but costs increased 18% due to expansion. Launched in 3 new markets. Customer satisfaction at all-time high of 94%.`,this.session=new m,this.session.setModel(s(`openai`,`gpt-4o-mini`)),this.agentInterface=new o,this.agentInterface.session=this.session,this.agentInterface.enableAttachments=!1,this.agentInterface.enableModelSelector=!0,this.agentInterface.enableThinking=!1,this.agentInterface.style.width=`100%`,this.agentInterface.style.height=`100%`,this.session.setSystemPrompt(`You are a helpful assistant. Follow the instructions exactly and be concise.`),this.session.clearMessages()}async runStep(e){this.isProcessing||e<0||e>=this.steps.length||e>0&&!this.completed.has(this.steps[e-1].id)||(this.isProcessing=!0,await this.agentInterface.sendMessage(this.steps[e].prompt),this.completed.add(this.steps[e].id),this.currentStep=e+1,this.isProcessing=!1)}resetFlow(){this.currentStep=0,this.completed.clear(),this.session.clearMessages()}renderContentPanel(){return a`<div class="w-full h-full p-4 pb-4">${this.agentInterface}</div>`}renderLeftDemoPanel(){return a`
