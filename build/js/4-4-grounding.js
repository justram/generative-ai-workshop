...
\`\`\`
${_}
\`\`\``,reference:`${v} documentation`}}],this.session=new m,this.session.setModel(s(`openai`,`gpt-4o-mini`)),this.agentInterface=new o,this.agentInterface.session=this.session,this.agentInterface.enableAttachments=!1,this.agentInterface.enableModelSelector=!0,this.agentInterface.enableThinking=!1,this.agentInterface.style.width=`100%`,this.agentInterface.style.height=`100%`}async runExample(e,t){this.selectedExample=e.id,this.useGrounding=t;let n=t?e.withGrounding.systemPrompt:e.withoutGrounding.systemPrompt;this.session.setSystemPrompt(n),this.session.clearMessages(),await this.agentInterface.sendMessage(e.question)}renderContentPanel(){return a`<div class="w-full h-full p-4 pb-4">${this.agentInterface}</div>`}renderLeftDemoPanel(){let t=this.examples.find(e=>e.id===this.selectedExample);return a`
