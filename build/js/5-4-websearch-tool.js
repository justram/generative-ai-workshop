...
import { DemoBase } from "./DemoBase-7724hyNv.js?v=section5-websearch-matt1";
...
let WebsearchToolDemo = class extends DemoBase {
	constructor() {
		...
		this.searchQuestions = [
			...
			},
			{
				...
				prompt: i18n(`請判斷公開搜尋結果中，Jheng-Hong (Matt) Yang 與 Stencilzeit 的關係是否能被穩定確認？請說明信心程度。`),
	...
	async runQuestion(question, useSearch) {
		...
		try {
			...
			this.session.appendMessage({
				role: `assistant`,
				content: [{ type: `text`, text: `${i18n(`搜尋流程失敗：`)}${error.message || String(error)}` }],
				timestamp: Date.now(),
			});
	...
	async planSearchQuery(question) {
		...
		plannerSession.setSystemPrompt(`你是 web_search client 的查詢規劃器。請把使用者問題轉成搜尋工具參數，不要回答問題本身。格式只能是 {"query":"搜尋字串","num_results":5}。搜尋字串要保留專有名詞、帳號、組織名稱與可能的英文拼法。`);
...
最後一行必須完全寫成：查詢時間：${payload.searchedAt}`;
	...
	async executeSearch(query) {
		...
		this.session.appendMessage({
			role: `assistant`,
			content: [{ type: `toolCall`, id: toolCallId, name: `web_search`, arguments: { query, num_results: 5 } }],
			api: `local-tool-bridge`,
			provider: `local`,
			model: `web_search`,
			usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } },
	...
	removeAbortedPlaceholders() {
		const messages = this.session.state.messages.filter((message) => {
			...
			const abortedText = message.content?.some((part) => part.type === `text` && /請求已中止|呼叫已中止|沒有結果|aborted/i.test(part.text));
	...
	clearModelStreamUi() {
		...
		this.agentInterface?.querySelectorAll?.(`div`)?.forEach((element) => {
			if (/^(請求已中止|呼叫已中止；沒有結果。|Request aborted)$/.test(element.innerText?.trim() ?? ``)) element.remove();
		});
	...
	renderProgressMarker(active, done, index) {
		if (done) return x`<div class="grid h-7 w-7 place-items-center rounded-full bg-emerald-500 text-xs font-bold text-black">✓</div>`;
		if (active) {
			return x`
	...
	renderQueryPreview() {
		...
		return x`
			${this.liveQuery
				...
				: i18n(`還沒收到模型輸出前，先看 web_search() 期待的參數形狀；模型要先把問題改寫成查詢字串。`)}
			...
			<div class="rounded-lg border ${isDone ? `border-border bg-card` : `border-primary/40 bg-primary/5`} p-3">
				...
				<details class="mt-3" ?open=${this.traceExpanded} @toggle=${(event) => (this.traceExpanded = event.currentTarget.open)}>
					...
					${this.renderPipelineStep(
						...
						1,
						i18n(`模型改寫查詢`),
						i18n(`學生問的是自然語言；模型先把它改成搜尋引擎比較懂的 query。這個草稿不進聊天紀錄，避免把內部格式誤當答案。`),
					...
					${this.renderPipelineStep(
						...
						4,
						i18n(`檢查來源與信心`),
						i18n(`最後一步不是相信答案，而是回頭看來源：profile、DBLP、LinkedIn 片段各自能支持什麼？`),
						isDone ? i18n(`流程已完成。請比較聊天回答與左側來源，找出哪些句子是可查證、哪些只是低信心推論。`) : i18n(`等待模型整理完答案。`),
						false,
					)}
					</div>
				</details>
