...
let m = class extends f {
	constructor() {
		...
		this.session = new p();
		this.session.setSystemPrompt(t(`你是一位協助分析多份文件的助理。請只根據提供的文件回答；如果答案需要跨文件比對，請明確指出依據來自哪份文件。`));
	...
	handleEditorAttachmentsChange(e) {
		...
		this.currentChallenge = {
			...
			question: this.agentInterface?._messageEditor?.value || t(`請輸入一個需要同時讀多份文件的問題。`),
			choices: {},
			answer: ``,
			explanation: t(`自訂上傳沒有預設參考答案。教學重點是檢查 payload 裡是否真的包含多份文件，以及模型回答是否明確標示來源。`),
		};
				...
				<div class="overflow-y-auto border-t border-border p-4 overscroll-contain" style="max-height:42vh">
					...
					<div class="grid gap-3">
						...
						${this.renderPipelineStep(
							...
							2,
							t(`各自抽成文字`),
							t(`這裡示範文字文件的抽取結果。真實產品若是 PDF、Word 或掃描圖，也會先形成可送進模型的文字片段。`),
	...
	renderAttachmentListOutcome() {
		...
		return i`
			<div class="grid gap-2">
				${this.currentAttachments.map((e, n) => i`
		...
		try {
			...
			this.challenges = [
				...
				},
				{
					...
					question: i.question,
					choices: { A: i.choice_A, B: i.choice_B, C: i.choice_C, D: i.choice_D },
					answer: i.answer,
