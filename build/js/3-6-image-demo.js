...
import { Card as s, CardContent as c, CardHeader as l, CardTitle as u } from "./Textarea-DCZnYrSo.js";
...
import { DemoBase as d } from "./DemoBase-7724hyNv.js";
import "./proxy-client-DO8A5rUF.js";
import { AgentSession as f } from "./agent-session-CtmWvP9t.js";
...
教學重點：模型可能讀對大意，卻把繁體字、單位或步驟文字轉成不精確的版本。`),
      ...
      },
      {
        ...
        prompts: [
          {
            text: t(`請讀標題欄：公司名稱、標題、圖號、型號、版本、頁碼分別是什麼？請逐欄回答，不要改寫。`),
...
教學重點：這題不是只看模型能不能讀字，而是看它會不會漏掉「水」這種小但關鍵的字。`),
          },
          {
            ...
            source: t(`資料集參考答案（CC-OCR blueprint_qa/answer/vqa_blueprint_100/10af6edcb983.txt）`),
            answer: t(`小型离心水泵剖面图

教學重點：gpt-5.4-mini thinking off 曾回答「小型離心泵剖面圖」，少了「水」。答案看起來通順，但不是資料集標註的正式名稱。`),
          },
          {
            text: t(`右側「零部件明細表」第 04、05 列：名稱、件號、數量、材料、規格、備註、腳注分別是什麼？請保留手寫註記。`),
            source: t(`參考答案（依 CC-OCR 圖面標註整理）`),
            answer: t(`04：名稱 支架；件號 FK39972H；數量 4；材料 Q235；規格 20x40；備註 手寫「加硬」；腳注 †
...
教學重點：工程圖常混合印刷字、手寫註記、符號腳注。模型即使大致讀對，也需要人工核對欄位對齊。`),
    ...
    this.session = new f();
    this.session.setSystemPrompt(t(`你是一位協助分析圖片的助理。請準確描述你看到的內容，並根據影像回答問題。`));
  ...
  async loadImageWithPrompt(e, t) {
    ...
    try {
      if (this.session.state.messages.some(t => t.role === `user` && Array.isArray(t.attachments) && t.attachments.some(t => t?.type === `image` && t?.fileName === e.file))) {
