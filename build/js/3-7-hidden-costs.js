...
例如，你可以提醒使用者：這段 system prompt 本身也會在每一輪訊息中消耗 token。`),this.session.setThinkingLevel(`off`),this.agentInterface=new i,this.agentInterface.session=this.session,this.agentInterface.enableAttachments=!0,this.agentInterface.enableModelSelector=!0,this.agentInterface.enableThinking=!0,this.agentInterface.style.width=`100%`,this.agentInterface.style.height=`100%`,this.agentInterface.setInput(`嗨！可以幫我理解為什麼大型語言模型的成本會累積得這麼快嗎？`),this.unsubscribe=this.session.subscribe(e=>{e.type===`state-update`&&this.requestUpdate()})}renderContentPanel(){return r`<div class="w-full h-full p-4 pb-4">${this.agentInterface}</div>`}renderLeftDemoPanel(){let e=this.session.state.messages.filter(e=>e.role===`assistant`&&e.usage&&e.usage.cost),n=0,i=0,s=[];for(let t of e){let e=Number(t.usage.cost.total||0);s.push(e),n+=e,e>i&&(i=e)}return r`
