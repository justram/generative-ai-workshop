...
文字：`),
        ...
        },
        {
          id: `sentiment`,
          name: n(`情緒分析`),
          prompt: n(`分析這段文字的情緒。請用 -1（非常負面）到 1（非常正面）評分，並說明原因：`),
          sampleInput: n(
...
回饋：`),
        ...
        },
        {
          id: `keywords`,
          name: n(`關鍵字抽取`),
          prompt: n(`請從這段文字抽出 5 個最重要的關鍵字或片語：`),
          sampleInput: this.defaultText,
        },
        {
          id: `simplify`,
          name: n(`簡化說明`),
          prompt: n(`請用 10 歲小孩也聽得懂的方式解釋這段文字：`),
          sampleInput: n(`量子運算利用疊加與糾纏等量子力學現象，執行傳統電腦難以處理的計算。`),
        },
        {
          id: `formal`,
          name: n(`正式化`),
          prompt: n(`請改寫成正式的學術語氣：`),
          sampleInput: n(
...
(t([r()], f.prototype, `selectedTask`, void 0),
  (f = t([i(`nlp-tasks-demo`)], f)),
  (document.body.innerHTML = `<nlp-tasks-demo></nlp-tasks-demo>`));
export { f as NLPTasksDemo };
