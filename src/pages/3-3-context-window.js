...
import { AgentInterface as a } from "../workshop-runtime/app-C9nW8ndw.js";
import {
  Badge as o,
  Card as s,
  CardContent as c,
...
import { getAuthToken as d } from "../workshop-runtime/auth-token-Dkh_JH49.js";
...
let m = class extends f {
  constructor() {
    ...
    const e = [
      {
        role: `user`,
        content: [{ type: `text`, text: t(`我叫 Mario。你今天好嗎？`) }],
      },
      {
        role: `assistant`,
        content: [
          {
            type: `text`,
            text: t(`Mario 你好！很高興認識你。我很好，謝謝關心！你今天過得如何？`),
          },
        ],
          ...
          this.droppedMessages.length > 0
            ...
            : i``
				...
				${s(i`
					...
					${c(
            i`${
              this.actualMessages.length > 0
                ? this.actualMessages.map(
                    (e) => i`
										<div class="p-2 rounded-md bg-card/50 mb-1">
											<div class="text-[10px] uppercase text-muted-foreground">${e.role === `user` ? t(`User`) : e.role === `assistant` ? t(`Assistant`) : e.role}</div>
