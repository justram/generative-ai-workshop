...
const SELF_HOSTING_LAB_EN = {
  ...
  "這頁不是排行榜。先選一台常見機器，再調整模型、量化、上下文與同時使用人數，看「能放下」和「值得買」是不是同一件事。":
    "This page is not a ranking. Pick a common machine, then adjust model size, quantization, context, and concurrent users to see whether “it fits” and “it is worth buying” are the same thing.",
  ...
  "M5 Max 可到 128GB unified memory；價格依螢幕尺寸、SSD 與客製化而變。":
    "M5 Max can be configured up to 128GB unified memory; price depends on display size, SSD, and configuration.",
  "把它當成特殊工作站，不是一般採購基準。":
    "Treat it as a special workstation, not a normal purchasing baseline.",
  "128GB coherent unified memory；定位接近個人 AI supercomputer。":
    "128GB coherent unified memory; positioned close to a personal AI supercomputer.",
  "許多機器可把部分記憶體切給 iGPU，常見說法是最高約 96GB VRAM 類配置。":
    "Many machines can reserve part of memory for the iGPU; a common claim is up to about a 96GB VRAM-like configuration.",
  "常被拿來和 RTX 5090 類消費卡比較；差價主要買 VRAM、ECC、驅動與保固。":
  ...
  "8k 對話或文件會保留的 KV cache。":
    "KV cache kept for an 8k conversation or document.",
  "1 個同時對話時，KV cache 會跟著放大。":
    "With 1 concurrent conversation, KV cache scales with it.",
  "框架、暫存、batch 與系統本身需要的空間。":
    "Space needed by the framework, temporary buffers, batching, and the OS itself.",
  餘裕充足: "Plenty of headroom",
  "可以做較長上下文、較大 batch 或更多實驗，但仍要實測速度。":
    "Can support longer context, larger batches, or more experiments, but speed still needs testing.",
  "可以跑，餘裕有限": "Runs, but headroom is limited",
  "適合 demo 或單人測試；正式服務要降低上下文、縮小模型或減少併發。":
    "Fine for demos or solo testing; production service needs shorter context, a smaller model, or fewer concurrent users.",
  會爆或嚴重換頁: "Will run out of memory or page badly",
  "這組設定不適合這台機器。先降量化、縮上下文、改小模型，或換雲端/API。":
    "This setup does not fit this machine. Lower precision, shorten context, use a smaller model, or switch to cloud/API.",
  適合: "Good fit",
  要小心: "Watch out",
  "帶著走的單人開發機。適合本機 demo、課堂、測小到中型模型，或把 GPT 工作流搬一部分到本機。":
    "Portable single-user development machine. Good for local demos, classrooms, small-to-mid model tests, or moving part of a GPT workflow locally.",
  "128GB 不是 128GB VRAM。系統、瀏覽器、IDE 和模型引擎都會吃 unified memory；長上下文或多人服務會很快卡住。":
    "128GB is not 128GB of VRAM. The OS, browser, IDE, and model runtime all consume unified memory; long context or multi-user service will get tight quickly.",
  "想在單機上放非常大的權重、長上下文或大批文件實驗。強項是容量與低功耗。":
    "Good for fitting very large weights, long context, or large document experiments on one machine. Its strength is capacity and low power.",
  "512GB 配置在 2026 年供貨很不穩，甚至曾從 Apple 官網選配項目消失。能放下不代表 tokens/sec 會像資料中心 GPU。":
    "The 512GB configuration has had unstable availability in 2026 and even disappeared from Apple configuration pages. Fitting the model does not mean data-center-GPU tokens/sec.",
  "想用 NVIDIA 軟體棧、測接近部署端的流程，並在桌上跑較大的模型原型。":
    "Good for using the NVIDIA software stack, testing deployment-like workflows, and prototyping larger local models on a desk.",
  "是 AI dev kit，不是萬能桌機。價格、供貨和 ARM/NVIDIA 軟體相容性都要先確認。":
    "It is an AI dev kit, not a universal desktop. Confirm price, availability, and ARM/NVIDIA software compatibility first.",
  "預算有限但想要 128GB 級共享記憶體的小型桌機。適合探索、教學和非 NVIDIA 路線。":
    "A small desktop path for limited budgets that still want 128GB-class shared memory. Good for exploration, teaching, and non-NVIDIA routes.",
  "ROCm / Vulkan / llama.cpp 支援要看工具；有些工作流仍然是 NVIDIA CUDA 最省事。":
    "ROCm / Vulkan / llama.cpp support depends on the tool; some workflows are still easiest on NVIDIA CUDA.",
  "需要 CUDA、生產級驅動、ECC VRAM、工作站穩定性與較高頻寬。適合正式 PoC 或小型服務。":
    "Good when you need CUDA, production drivers, ECC VRAM, workstation stability, and higher bandwidth. Suitable for formal PoCs or small services.",
  "這通常只是一張卡，還要主機板、電源、散熱、機殼與維運。買卡不等於買到服務。":
    "This is usually only a card; you still need motherboard, power, cooling, case, and operations. Buying a GPU is not buying a service.",
  要跑的模型設定: "Model settings to test",
  模型架構: "Model architecture",
  "Dense 看總參數；MoE 要分開看總權重與每 token 啟用參數。":
    "Dense models are judged by total parameters; MoE needs total weights and per-token active parameters separated.",
  "每個 token 都跑整個模型": "Every token runs through the whole model",
  "載入全部權重，但每個 token 只啟用一部分專家":
    "Loads all weights, but each token activates only some experts",
  常見模型範例: "Common model examples",
  "點一個範例，看 dense 與 MoE 的記憶體取捨差在哪裡。":
    "Click a preset to see how dense and MoE memory tradeoffs differ.",
  ...
  "MoE 可以每次只啟用部分專家，但整包權重仍然要放得下。":
    "MoE can activate only some experts per token, but the full weight package still has to fit.",
  "Dense 模型每個 token 幾乎都會用到整個模型。":
    "Dense models use almost the whole model for every token.",
  小模型: "Small model",
  大型權重: "Large weights",
  "每 token 啟用參數": "Active parameters per token",
  "這比較接近速度與單 token 計算量；但不代表總權重可以不用載入。":
    "This better approximates speed and per-token compute; it does not mean total weights need not be loaded.",
  少量啟用: "Low activation",
  大量啟用: "High activation",
  量化位元: "Quantization bits",
  "量化不是連續旋鈕，而是常見格式選擇。越低越省記憶體，也越可能犧牲品質。":
    "Quantization is not a continuous knob; it is a format choice. Lower bits save memory and may sacrifice quality.",
  "極省，但品質風險最高":
    "Saves the most memory, with the highest quality risk",
  ...
  "上下文越長，每個同時對話都會多吃一份 KV cache。正式服務時，這通常比 demo 更可怕。":
    "Longer context adds KV cache for every concurrent conversation. In real service, this is often scarier than in a demo.",
  "token 是模型切文字的單位。頁數只是粗估；PDF、表格、程式碼、OCR 都會吃不一樣多。":
    "A token is the unit the model uses to split text. Page counts are only rough estimates; PDFs, tables, code, and OCR all consume tokens differently.",
  "1M token 是百萬級上下文，適合展示「能塞很多」和「成本也很高」的取捨。它不是萬能記憶體；找重點仍需要結構化流程。":
    "A 1M-token context demonstrates the tradeoff between fitting a lot and paying a lot. It is not magic memory; finding the important parts still needs structure.",
  短文件: "Short document",
  ...
  "幾頁 PDF、幾篇新聞、短問答、單頁 OCR、單一程式片段":
    "A few PDF pages, a few news articles, a short Q&A, one OCR page, or one code snippet",
  "短報告、一章教材、幾頁合約或講義、小功能相關檔案":
    "A short report, one textbook chapter, a few contract or handout pages, or files for a small feature",
  "完整報告節選、幾章教材、多頁表格 OCR、小模組上下文":
    "Report excerpts, several textbook chapters, multi-page table OCR, or a small module context",
  "長報告、多文件節選、論文草稿與參考段落、跨檔案除錯":
    "Long reports, multi-document excerpts, paper drafts with references, and cross-file debugging",
  "短書章節、大型規格書、大量 OCR 文字、中型專案局部上下文":
    "Short book chapters, large specs, lots of OCR text, or partial context for a medium project",
  "一本短書、厚規格書、大量 OCR 結果、中型專案較大上下文":
    "A short book, thick spec, lots of OCR output, or larger context for a medium project",
  "多份長文件、長書稿參考材料、較大專案多模組上下文":
    "Multiple long documents, long-manuscript reference material, or multi-module context for a larger project",
  "大量文件或程式資料；能放很多，但更需要摘要、索引與驗證":
    "Lots of documents or code; it fits a lot, but needs summarization, indexing, and verification even more",
  "這不是固定頁數換算。PDF 排版、表格、程式碼、OCR 錯字與圖片說明都會改變實際 token 數。":
    "This is not a fixed page-count conversion. PDF layout, tables, code, OCR errors, and image captions all change the real token count.",
  ...
  "大型專案的一部分，但仍不是整個世界":
    "Part of a large project, but still not the whole world",
  同時使用人數: "Concurrent users",
  "不是帳號總數，而是同一時間真的在推論的對話數。多人使用會放大 KV cache 與延遲。":
    "Not total accounts; this is the number of conversations actually running inference at the same time. More users amplify KV cache and latency.",
  "單人 demo": "Solo demo",
  小型服務: "Small service",
  採購時不要只看容量: "Do not buy by capacity alone",
  容量決定能不能放下: "Capacity decides whether it fits",
  "模型權重、上下文、多人使用都會吃記憶體。能放下只是第一關。":
    "Model weights, context, and multiple users all consume memory. Fitting is only the first gate.",
  頻寬與軟體決定跑得順不順:
    "Bandwidth and software decide whether it runs smoothly",
  "CUDA、Metal、ROCm、MLX、llama.cpp、vLLM 支援不同。工具鏈不順，硬體規格再漂亮也會卡。":
    "CUDA, Metal, ROCm, MLX, llama.cpp, and vLLM differ in support. If the toolchain is rough, nice hardware specs still get stuck.",
  價格不是總成本: "Price is not total cost",
  "工作站還要算電力、散熱、保固、備援、監控、更新與誰負責排錯。":
    "Workstations also need power, cooling, warranty, backup, monitoring, updates, and someone responsible for debugging.",
  "價格為 2026-05 公開美國零售或媒體整理的近似值，實際會因地區、供貨、SSD、稅金與教育/企業方案改變。":
    "Prices are approximate public US retail or media-reported figures as of 2026-05. Actual prices vary by region, availability, SSD, tax, and education/enterprise programs.",
  ...
  "越不能外流，越有本機或私有部署的理由。":
    "The less data can leave, the stronger the case for local or private deployment.",
  "資料限制不高時，先用 API 會比較快。":
    "When data restrictions are low, starting with an API is usually faster.",
  "固定且大量的工作，才比較可能攤平自架成本。":
    "Fixed, high-volume work is where self-hosting cost is more likely to amortize.",
  "用量還小時，硬體與維運通常比 token 帳單更貴。":
    "When usage is still small, hardware and operations usually cost more than the token bill.",
  "有人能更新、監控與排錯，自架才不會卡在第一週。":
    "Self-hosting only survives week one when someone can update, monitor, and debug it.",
  "維運能力不足時，模型能跑不代表服務能穩定交付。":
    "If operations capability is weak, a model that runs is not the same as a service that ships reliably.",
  "品質門檻越高，越要保留 GPT/API 作為基準或備援。":
    "The higher the quality bar, the more you should keep GPT/API as a baseline or fallback.",
  "品質需求較鬆時，可以先用小模型驗證流程。":
    "When quality requirements are looser, use a small model first to validate the workflow.",
  分項記憶體用量: "Memory usage by component",
  "模型本體。模型越大、量化位元越高，這塊越大。":
    "The model itself. Larger models and higher quantization bits make this part bigger.",
  各列顯示自己的變化幅度: "Each row shows its own change scale.",
  估算結果: "Estimate",
  "粗估，實際依引擎與硬體而變":
    "Rough estimate; actual numbers vary by engine and hardware.",
  總記憶體預算: "Total memory budget",
  "上下文越長仍會吃記憶體；MoE 的啟用參數不會讓 KV cache 消失。":
    "Longer context still consumes memory; MoE active parameters do not make KV cache disappear.",
  "上下文越長，推論時保留的中間狀態越多。":
    "The longer the context, the more intermediate state must be kept during inference.",
  "框架、暫存、batch 與系統本身都需要額外空間。":
    "Frameworks, temporary buffers, batching, and the OS itself all need extra space.",
  怎麼判斷模型大小: "How to judge model size",
  "7B～14B": "7B-14B",
  "30B～32B": "30B-32B",
  "適合課堂、摘要、分類、簡單客服；容易在筆電上跑。":
    "Good for class demos, summarization, classification, and simple support; easy to run on a laptop.",
  "品質明顯提升，但硬體門檻上來；適合小團隊 PoC。":
    "Quality improves noticeably, but hardware requirements rise; good for small-team PoCs.",
  "不是不能跑，而是要開始像營運服務一樣管理成本與併發。":
    "It is not impossible to run, but you must manage cost and concurrency like an operational service.",
  "不要只看「能不能跑」。真正要確認的是：回應速度是否可接受、上下文是否夠用、答案品質是否能通過你的任務測試。":
    "Do not only ask whether it runs. Check response speed, context adequacy, and whether answer quality passes your task tests.",
  偶爾: "Occasional",
  ...
  "你已經有明確的資料、用量或控制需求。下一步不是先買 GPU，而是用一個小任務做可驗證的 PoC。":
    "You already have a clear data, volume, or control need. The next step is not buying a GPU first; it is a verifiable PoC on one small task.",
  先做混合架構: "Start with a hybrid architecture",
  "把資料敏感或固定重複的工作放本機；高難度推理、低頻任務仍用 GPT/API 當基準。":
    "Move sensitive-data or fixed repetitive work locally; keep hard reasoning and low-frequency tasks on GPT/API as the baseline.",
  "先用 API 驗證需求": "Validate the need with an API first",
  "目前自架很容易變成維運負擔。先把任務、成本、速度和品質基準跑清楚。":
    "Right now, self-hosting can easily become an operations burden. First clarify the task, cost, speed, and quality baseline.",
  "自架適不適合你？": "Is self-hosting a fit for you?",
  "自架不是把 ChatGPT 搬回家。這裡先用四個條件判斷：資料能不能出去、用量夠不夠固定、誰能維護、品質門檻有多高。":
    "Self-hosting is not bringing ChatGPT home. Start with four questions: can the data leave, is usage fixed enough, who maintains it, and how high is the quality bar?",
  你的情境: "Your situation",
  每天大概會問幾次: "Roughly how many requests per day",
  "不用精準，重點是看它是偶爾使用，還是固定大量工作。":
    "It does not need to be precise; the point is whether usage is occasional or fixed high-volume work.",
  資料能不能丟到外部服務: "Can the data go to an external service?",
  "公開資料、內部文件、客戶資料與機密資料，部署選擇會完全不同。":
    "Public data, internal documents, customer data, and confidential data lead to very different deployment choices.",
  誰可以維護這套東西: "Who can maintain this system?",
  "自架之後，更新、監控、權限、安全與故障排除都要有人負責。":
    "After self-hosting, someone owns updates, monitoring, permissions, security, and incident response.",
  "答案品質需要多接近 GPT": "How close must answer quality be to GPT?",
  "品質門檻越高，越不能只看模型能不能啟動，要拿真實任務測。":
    "The higher the quality bar, the less you can rely on “it starts”; you need real task tests.",
  目前判斷: "Current read",
  這頁要帶走的判斷: "What to take away",
  "不是免費版 ChatGPT": "It is not free ChatGPT",
  "自架是把 token 帳單換成硬體、維運、更新、安全與品質風險。":
    "Self-hosting trades the token bill for hardware, operations, updates, security, and quality risk.",
  先找穩定任務: "Find a stable task first",
  "先用 API 跑出穩定需求，再決定哪些固定工作值得搬到本機或私有雲。":
    "Use an API to prove stable demand first, then decide which fixed tasks are worth moving to local or private cloud.",
  失敗時誰負責: "Who owns failures?",
  "不要只問能不能跑，也要問誰維護、怎麼更新、如何監控、答案錯了誰處理。":
    "Do not only ask whether it runs; ask who maintains it, how it updates, how it is monitored, and who handles wrong answers.",
  實作示範路線: "Hands-on demo path",
  起手式: "Starter command",
  "模型：要跑多大、需要多少記憶體":
    "Models: size, architecture, and memory budget",
  "軟體堆疊：模型之外還需要什麼":
    "Software stack: what else you need beyond the model",
  軟體堆疊不是只有模型: "The software stack is not just the model",
  "模型只是權重檔。真正讓它能被使用的，是模型管理、推論引擎、API server、UI 與維運責任。":
    "A model is only a weight file. What makes it usable is model management, the inference engine, an API server, UI, and operational responsibility.",
  ...
  "學習重點：不要只問「哪個工具能跑模型」。要問這個工具幫你包了哪些層，哪些層最後仍然要由你或團隊負責。":
    "Learning point: do not only ask which tool can run a model. Ask which layers the tool wraps for you, and which layers you or your team still own.",
  個人本機服務: "Personal local service",
  "最快讓本機模型跑起來，通常同時處理下載、啟動與本機 API。":
    "The fastest way to get a local model running; usually handles download, startup, and a local API together.",
  "個人開發、課堂示範、小型 PoC":
    "Personal development, classroom demos, small PoCs",
  ...
  "App 用 OpenAI-compatible API 連上去":
    "The app connects through an OpenAI-compatible API",
  "可用 Ollama CLI、第三方聊天 UI，或你自己的 app。":
    "Use the Ollama CLI, a third-party chat UI, or your own app.",
  模型檔: "Model file",
  "Ollama 幫你管理模型名稱、下載與快取。":
    "Ollama manages model names, downloads, and cache for you.",
  "主要是個人機器上的版本、模型更新與記憶體限制。":
    "Mostly versioning, model updates, and memory limits on a personal machine.",
  "內建聊天介面，適合不用命令列也能試模型。":
    "Built-in chat UI, good for trying models without a command line.",
  "GUI 顯示模型與量化格式，降低下載門檻。":
    "The GUI shows models and quantization formats, lowering the download barrier.",
  "桌面 app 包好推論流程，適合單機互動。":
    "The desktop app wraps the inference flow, suitable for single-machine interaction.",
  "可開 OpenAI-compatible local server 給其他工具連。":
    "Can start an OpenAI-compatible local server for other tools to connect to.",
  "正式多人使用、權限、監控與併發不是它的主要強項。":
    "Production multi-user access, permissions, monitoring, and concurrency are not its main strengths.",
  "通常沒有給一般使用者的 UI，要由你的產品或內部工具提供。":
    "Usually no end-user UI; your product or internal tool must provide it.",
  "自行管理 Hugging Face 權重、量化格式與版本。":
    "Manage Hugging Face weights, quantization formats, and versions yourself.",
  "專注高吞吐與服務化，例如 batching、prefix cache、並行。":
    "Focused on high-throughput serving, such as batching, prefix cache, and parallelism.",
  "提供 OpenAI-compatible 或自家 API 給產品呼叫。":
    "Provides an OpenAI-compatible or custom API for the product to call.",
  "權限、監控、日誌、擴縮、失敗重啟與成本都要納入。":
    "Permissions, monitoring, logs, scaling, failure recovery, and cost must all be included.",
  "桌面 GUI": "Desktop GUI",
  "用圖形介面找模型、下載、聊天，也可以一鍵啟動本機 server。":
    "Use a graphical interface to find models, download them, chat, and start a local server with one click.",
  "非工程背景、模型試用、桌面 demo":
    "Non-engineering users, model trials, desktop demos",
  ...
  "需要整合時啟動 local server":
    "Start a local server when integration is needed",
  服務化推論: "Serving inference",
  "把模型當成服務部署，重點是吞吐、併發、batching、監控與權限。":
    "Deploy the model as a service, with focus on throughput, concurrency, batching, monitoring, and permissions.",
  "團隊服務、多人併發、正式環境":
    "Team services, concurrent users, production-like environments",
  "準備 GPU / 容器 / 權重": "Prepare GPU / container / weights",
  "啟動推論 server": "Start the inference server",
  "設定 batching、KV cache 與併發":
    "Configure batching, KV cache, and concurrency",
  "接上權限、監控、日誌與備援":
    "Connect permissions, monitoring, logs, and failover",
  ...
  準備: "Prepare",
  執行: "Run",
  "用 Ollama 或 LM Studio 開一個中小型模型，問同一組課堂題與工作題。":
    "Use Ollama or LM Studio to run a small-to-mid model, then ask the same classroom and work questions.",
  紀錄: "Record",
  "留下延遲、記憶體、錯誤訊息與實際輸出，不只截一張成功畫面。":
    "Keep latency, memory, error messages, and actual outputs, not just a screenshot of success.",
  "HTTP 狀態與串流是否正常":
    "Whether HTTP status and streaming behave correctly",
  "模型名稱、錯誤訊息、timeout 是否可控":
    "Whether model names, error messages, and timeouts are controllable",
  "工具呼叫與 JSON 輸出是否和 GPT 行為相容":
    "Whether tool calls and JSON output are compatible with GPT behavior",
  "不要只看「有回應」": "Do not just check that it responds",
  "這頁不是在模擬模型輸出，而是整理你真的跑 Ollama、LM Studio 或本機 API 時要驗收的證據。成功畫面很便宜；可重複的證據比較有用。":
    "This page is not simulating model output; it organizes the evidence you should collect when really running Ollama, LM Studio, or a local API. A success screenshot is cheap; repeatable evidence is useful.",
...
class SelfHostingLab extends HTMLElement {
    ...
    const translate = (value) => {
      ...
      if (trimmed.startsWith("目前選到：")) {
        ...
        const translatedSelected =
          SELF_HOSTING_LAB_EN[selected] || i18n(selected);
        return value.replace(trimmed, `Selected: ${translatedSelected}`);
      }
      if (trimmed.includes(" · ")) {
        const translatedParts = trimmed
          .split(" · ")
          .map((part) => SELF_HOSTING_LAB_EN[part] || i18n(part));
  ...
  render() {
    ...
    this.querySelectorAll("button[data-key]").forEach((button) => {
      button.addEventListener("click", (event) => {
        const value =
          event.currentTarget.dataset.numeric === "true"
  ...
  effectiveActiveParams() {
    return this.state.architecture === "moe"
      ? Math.min(this.state.activeParams, this.state.params)
      : this.state.params;
  }
  ...
  modelMemoryBreakdown() {
    ...
    const kvParamProxy =
      this.state.architecture === "moe"
        ? Math.max(activeParams, Math.min(this.state.params, 14))
        : this.state.params;
    const kv =
      this.state.context *
      (kvParamProxy <= 14 ? 0.25 : kvParamProxy <= 35 ? 0.45 : 0.8);
  ...
  contextGuide() {
    ...
    if (k <= 8) {
      return {
        label: "短文件",
        plain:
          "大概像幾頁 PDF、幾篇新聞，或一小段程式碼。適合短問答、短翻譯、單頁 OCR。",
        tasks: [
          ["QA", "一小份文件內找答案"],
          ["翻譯／寫作", "短文或一小節手冊"],
          ["OCR", "單頁表格或收據"],
    ...
    if (k <= 16) {
      return {
        label: "中短文件",
        plain:
          "大概像一份短報告、一章教材，或幾個程式檔。適合摘要、比對、翻譯一段文件。",
        tasks: [
          ["QA", "短報告或單章教材"],
          ["翻譯／寫作", "幾頁合約、講義或草稿"],
          ["OCR", "少量掃描頁面"],
    ...
    if (k <= 32) {
      return {
        label: "中型文件",
        plain:
          "大概像一份較完整的 PDF 節選、幾章教材，或一組相關程式檔。適合多段 QA、摘要與改寫。",
        tasks: [
          ["QA", "一份中型報告"],
          ["翻譯／寫作", "較長文件，但仍要分段檢查"],
          ["OCR", "多頁表格與段落"],
    ...
    if (k <= 64) {
      return {
        label: "長文件",
        plain:
          "大概像長報告、大量 OCR 文字，或多個程式檔。仍不等於整本書或整個專案；任務越複雜越需要分段。",
        tasks: [
          ["QA", "長報告或多文件節選"],
          ["翻譯／寫作", "論文草稿與參考段落"],
          ["OCR", "多頁財報或表格"],
    ...
    if (k <= 128) {
      return {
        label: "超長文件",
        plain:
          "大概像短書、大型規格書、大量 OCR 結果，或較多程式檔。看似很大，但完整書籍、完整專案、長期對話仍可能超出範圍。",
        tasks: [
          ["QA", "長報告加多份附件"],
          ["翻譯／寫作", "短書章節或長篇草稿"],
          ["OCR", "大量掃描頁面節選"],
    ...
    if (k <= 256) {
      return {
        label: "大型資料包",
        plain:
          "大概像一本短書、厚規格書、較多掃描頁 OCR，或一個中型專案的局部資料。上下文變大，記憶體也會明顯增加。",
        tasks: [
          ["QA", "大型規格書與附件"],
          ["翻譯／寫作", "短書或長篇報告草稿"],
          ["OCR", "大量頁面抽取後的文字"],
    ...
    if (k <= 512) {
      return {
        label: "巨量資料包",
        plain:
          "可以放進非常多文字，但不代表模型會自動理解全部重點。越大的上下文越需要分段、索引、摘要與驗證策略。",
        tasks: [
          ["QA", "多份長文件與附件"],
          ["翻譯／寫作", "長書稿節選與參考材料"],
          ["OCR", "大量掃描文件抽取結果"],
  ...
  renderDemos() {
    const demos = {
      ...
      api: {
        title: "產品串接測試",
        intent:
          "確認本機模型能不能被現有 app 當成 OpenAI-compatible API 使用，而不是只能在聊天視窗裡玩。",
        action:
          "把 app 的 base URL 指到本機服務，送出一個會串流回覆的簡短任務。",
        ...
        decision:
          "API 打得通只是第一步；真正要看的是產品現有流程是否需要改 prompt、schema 或重試策略。",
        pitfall:
          "很多服務宣稱相容，但 function calling、structured output、streaming 細節會和 GPT 不同。",
      },
      eval: {
        title: "小型評測集",
        intent:
          "不要憑感覺說某個模型夠用。拿自己的真實任務做小型評測，才知道它會在哪裡失手。",
        action:
          "準備 20 題常見任務，讓 GPT 與本機模型跑同一份題目，逐題標成通過、部分通過、失敗。",
        ...
        decision:
          "如果失敗集中在低風險、可人工修正的工作，本機模型可能值得；如果錯在關鍵判斷，就要保留 GPT 或人工審核。",
        pitfall:
          "平均分數會遮住高風險失敗；一題財務單位看錯，可能比十題文案寫得順更重要。",
      },
    };
